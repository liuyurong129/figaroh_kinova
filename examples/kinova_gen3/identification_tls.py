...
identification_q_path = os.path.join(package_dirs, 'data','identification','trapezoidal_motion_7dof','q_trapezoid_real.csv')
identification_tau_path = os.path.join(package_dirs, 'data','identification','trapezoidal_motion_7dof','tau_trapezoid_real.csv')
identification_q_loaded_path = os.path.join(package_dirs, 'data','identification','trapezoidal_motion_7dof','q_trapezoid_real_weighted.csv')
identification_tau_loaded_path = os.path.join(package_dirs, 'data','identification','trapezoidal_motion_7dof','tau_trapezoid_real_weighted.csv')
...
with open(identification_q_loaded_path, 'r') as f:
    ...
    for row in csvreader:
        if ii == 0:
            ...
            q_with_load[ii - 1, :] = np.array([float(row[0]), float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5]), float(row[6])])
        ii += 1
...
# Print the size of the samples of the loaded and unloaded data
print("Size of samples without load:", q.shape)
print("Size of samples with load:", q_with_load.shape)
...
with open(identification_tau_loaded_path, 'r') as f:
    ...
    # tau_measured_with_load = tau_filt



# Define gain scale for torque data
print("Size of tau samples without load:", tau_measured.shape)
print("Size of tau samples with load:", tau_measured_with_load.shape)
gs_true = 1  # Torque scaling factor
vsa = (tau_measured / gs_true).reshape(-1, 1).flatten()  # Measured torque without load
vsb = (tau_measured_with_load / gs_true).reshape(-1, 1).flatten()  # Measured torque with load
print(f"nb_samples = {params_settings['nb_samples']}")
params_settings["nb_samples"] -= 2 * 5 * nbutter  # Adjust sample count for filtering
print(f"nb_samples = {params_settings['nb_samples']}")
print(f"Size of vsa: {vsa.shape}")
print(f"Size of vsb: {vsb.shape}")

quotient = len(vsa) // params_settings["nb_samples"]
remainder = len(vsa) % params_settings["nb_samples"]
...
# W_tot, V_norm, residue = build_total_regressor_current(W_base, W_base_with_load, W_with_load, tau_measured, tau_measured_with_load, params_standard, params_settings)
