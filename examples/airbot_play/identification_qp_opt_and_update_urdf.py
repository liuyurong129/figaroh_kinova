...
with open('examples/airbot_play/data/identification/trapezoidal_motion/identification_q_real.csv', 'r') as f:
...
for i in range(1, model.nq+1):  
    friction_coefficients[f'fv{i}'] = params_standard[f'fv{i}']
    friction_coefficients[f'fs{i}'] = params_standard[f'fs{i}']
print(friction_coefficients)
...
for i in range(len(q)): 
    ...
    for j in range(model.nq):  
        fv_key = f'fv{j+1}'
        fs_key = f'fs{j+1}'
        tau_fv = friction_coefficients[fv_key] * dq[i, j]

        tau_fs = friction_coefficients[fs_key] * np.sign(dq[i, j])
...
# for i in range(ddq.shape[1]):  # Iterate over each joint
#     plt.plot(ddq[:, i], label=f'Joint {i+1} Acceleration')

# plt.legend(title="Joint Acceleration")
# plt.xlabel('Samples')
...
print("When using trapezoidal trajectories the cond num is", np.linalg.cond(W_base))
...
with open('examples/airbot_play/data/identification/trapezoidal_motion/identification_tau_real.csv', 'r') as f:
    csvreader = csv.reader(f)
    next(csvreader) 
    tau_list = []
    ...
    # plt.legend(title="Joint Torque")
    # plt.xlabel('Samples')
    # plt.ylabel('Torque(Nm)')
    # plt.legend()
    # plt.show()

    tau_measured = np.array(tau_nofilt).T.flatten()
...
def total_mass_constraint(Phi):
    return np.sum(Phi[9::12]) - 3.42405  # Sum of all mass terms
...
updated_robot = Robot(
    'examples/airbot_play/data/urdf/updated_robot.urdf',
    package_dirs=package_dirs
)
updated_model = updated_robot.model
updated_data = updated_robot.data

with open('examples/airbot_play/config/airbot_play_config_updated.yaml', 'r') as f:
...
updated_friction_params = read_friction_params_from_urdf('examples/airbot_play/data/urdf/updated_robot.urdf')
