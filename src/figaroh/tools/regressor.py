...
def build_total_regressor_current_validate_inputs(W_b_u, W_b_l, W_l, I_u, I_l, param_standard_l, param):
    assert isinstance(W_b_u, np.ndarray), "W_b_u must be a numpy array"
    assert isinstance(W_b_l, np.ndarray), "W_b_l must be a numpy array"
    assert isinstance(W_l, np.ndarray), "W_l must be a numpy array"
    assert isinstance(I_u, np.ndarray) and I_u.ndim == 1, "I_u must be a 1D numpy array"
    assert isinstance(I_l, np.ndarray) and I_l.ndim == 1, "I_l must be a 1D numpy array"
    assert isinstance(param_standard_l, dict), "param_standard_l must be a dictionary"
    assert isinstance(param, dict), "param must be a dictionary"

    required_keys = ['nb_samples', 'mass_load', 'has_friction', 'has_actuator_inertia', 'which_body_loaded']
    for key in required_keys:
        assert key in param, f"Missing required key in param: '{key}'"

    nb_samples = param['nb_samples']
    total_length = len(I_u)
    assert len(I_u) == len(I_l), "I_u and I_l must have the same length"

    assert total_length % nb_samples == 0, "Length of I_u must be divisible by nb_samples"

    nb_joints = total_length // nb_samples

    assert W_b_u.shape[0] == W_b_l.shape[0] == W_l.shape[0] == total_length, \
        "All regressor matrices must have the same number of rows as I_u and I_l"

    if param['has_friction']:
        cols_required = (param['which_body_loaded'] + 1) * 12
        assert W_l.shape[1] >= cols_required, \
            f"W_l must have at least {cols_required} columns for friction parameters"
    elif param['has_actuator_inertia']:
        cols_required = (param['which_body_loaded'] + 1) * 14
        assert W_l.shape[1] >= cols_required, \
            f"W_l must have at least {cols_required} columns for actuator inertia parameters"
    else:
        cols_required = (param['which_body_loaded'] + 1) * 10
        assert W_l.shape[1] >= cols_required, \
            f"W_l must have at least {cols_required} columns for standard parameters"
...
def build_total_regressor_current(W_b_u, W_b_l,W_l, I_u, I_l,param_standard_l, param):
    ...
    # build_total_regressor_current_validate_inputs(W_b_u, W_b_l, W_l, I_u, I_l, param_standard_l, param)
